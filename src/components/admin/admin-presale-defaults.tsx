  ...
  async function save() {
    ...
    try {
      const results = await Promise.all(DEFAULTS.concat({ key: "presale_default_network", label: "", placeholder: "" }).map(async ({ key }) => {
        const response = await fetch("/api/admin/settings", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ key, value: key === "presale_default_network" ? "bsc" : values[key] ?? "" }) });
  ...
  return (
    <Card className="p-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div><Label className="text-xs">Network</Label><div className="mt-1 flex h-10 items-center rounded-md border bg-muted/30 px-3 text-sm font-medium">BNB Smart Chain (BSC / BEP20)</div></div>
