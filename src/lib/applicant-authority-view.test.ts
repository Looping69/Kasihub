...
describe("applicant authority presentation gates", () => {
  test.each([
    ...
    expect(applicantAuthorityView({ available: true, journey: staleJourney, reservation })).toEqual({
  ...
  test("never exposes reservation creation while a non-terminal reservation exists", () => {
    expect(applicantAuthorityView({ available: true, journey: journey({ allowedActions: ["create_reservation"] }), reservation }).canCreateReservation).toBe(false);
  });

  test.each(["cancelled", "expired"] as const)("exposes a new path after %s only when the backend explicitly permits it", (state) => {
    const denied = { available: true, journey: journey({ state, allowedActions: ["resume_application"], terminal: true }), reservation: null };
    const allowed = { available: true, journey: journey({ state, allowedActions: ["create_reservation"], terminal: true }), reservation: null };
  ...
  test("keeps financial presentation values exactly as returned by reservation authority", () => {
    expect(readApplicantAuthority({ journey: journey(), reservation }).reservation).toMatchObject({
