...
export async function POST(req: NextRequest) {
  ...
  try {
    return NextResponse.json(await encoreRequest("/admin/presale/coupons/policy", { method: "POST", body: JSON.stringify(await req.json()) }, token), { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
