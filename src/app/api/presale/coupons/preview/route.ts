...
export async function POST(req: NextRequest) {
  ...
  try {
    return NextResponse.json(await encoreRequest("/presale/coupons/preview", { method: "POST", body: JSON.stringify(await req.json()) }, token), { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
