...
- 2026-08-19 — Fixed the two still-open PR #26 investor-application review findings in a narrowly scoped follow-up: migration 8 adds the AES-256-GCM nonce, authentication tag and key-version columns for protected review notes and approval comments, with a migration contract test. Focused backend tests, Encore check, frontend typecheck, lint and production build passed. Invitation/session enforcement and every payment path remain unchanged; no deployment, application, approval, transaction or funds changed. Author: Klaasvaakie ( |╲ )
