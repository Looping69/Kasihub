...
- Deployed exact merged main revision `60a4057` to the isolated Vercel `kasihub-staging` project. Remote Next.js compilation, TypeScript, 72-page generation, and deployment completed successfully; deployment `dpl_FhwSumuf5X1xo6bXNwsCwZNdzFoi` is READY and aliased to `kasihub-staging.vercel.app`. Verified HTTP 200 for `/`, `/presale`, `/manifest.webmanifest`, and `/api/presale/campaigns`, with no error-level or HTTP 500 runtime logs in the post-deploy scan. Restored the primary checkout's Vercel link to the production `kasihub` project after staging release. Production was not redeployed or modified. Author: Klaasvaakie ( |╲ )

- Repaired the staging presale invitation regression reported from the shares page. Branch `Klaasvaakie/fix-presale-invite-links`, commit `a2d3af0`, now generates `/presale?invite=...` links and adds a compatibility `/presale/[invite]` redirect for already-copied links. Lint, TypeScript, 85 unit tests, production build, and all 12 browser journeys passed. Deployed READY staging artifact `dpl_3qcXqzh6gNGucFWzMRUMXRrvgKEh`; the exact reported URL now returns 307 to the supported query URL and finishes HTTP 200 with no Vercel 500s. Encore staging separately confirms that this specific token is invalid, expired, or belongs to an inactive campaign, so a fresh invitation is required even though the 404 is fixed. Production was untouched. Author: Klaasvaakie ( |╲ )

- Repaired the staging reservation HTTP 401 shown on a valid presale offer. Commit `10eac4c` on `Klaasvaakie/fix-presale-invite-links` now requires the frontend session and forwards its Encore token when creating a presale order. All 24 presale BFF contracts, lint, TypeScript, and the production build passed; deployed READY staging artifact `dpl_9WmoLUVrd47E7HXeTKYnnzmc1zn9`. The screenshot's fresh invitation subsequently returned Encore 403 after reload, meaning it expired or became inactive before a live post-deploy reservation could be submitted; no new order, payment instruction, or allocation was claimed. Production remained untouched. Author: Klaasvaakie ( |╲ )

- Implemented the staging onboarding-authority and investor-application boundary on `Klaasvaakie/staging-investor-onboarding-boundary`. KaSiPay-owned applicants now provide only KaSiHub login/contact details while provider identity and KYC results are expected through the provider API; international and local opt-out applicants use KaSiHub-owned KYC and USDT routing. Every presale buyer now submits a separate versioned investor application with tax residence, beneficial ownership, source-of-funds, bank, AML and suitability data; the complete snapshot is AES-256-GCM encrypted and only a minimal workflow summary remains queryable. All 86 frontend tests, TypeScript, lint and the Next.js production build pass; Encore compiles to the Docker integration boundary, while database integration execution remains blocked because Docker Desktop is not running. No staging or production deployment has yet occurred. Author: Klaasvaakie ( |╲ )

- Extended the public registration-policy endpoint to accept the explicit provider-versus-KaSiHub onboarding authority, allowing staging to prove the opt-out route without creating a member record. Author: Klaasvaakie ( |╲ )
